...
var NavigationMenuContext = createContext2();
...
var MenuRootContext = createContext4();
...
function Menu(props) {
  ...
  return <DomCollectionProvider><MenuContext.Provider value={context}><Show
  ...
  ><Popper
    ...
    {...others}
  /></Show></MenuContext.Provider></DomCollectionProvider>;
}
...
import { createContext as createContext5, useContext as useContext5 } from "solid-js";
var MenuItemContext = createContext5();
function useMenuItemContext() {
  const context = useContext5(MenuItemContext);
  if (context === void 0) {
    throw new Error(
...
function MenuItemBase(props) {
  ...
  const [descriptionId, setDescriptionId] = createSignal2();
  const [labelRef, setLabelRef] = createSignal2();
  ...
  const onPointerMove = (e) => {
    ...
    if (local.disabled) {
      ...
      if (!e.defaultPrevented) {
        focusWithoutScrolling2(e.currentTarget);
...
function MenuTrigger(props) {
  ...
  const isNativeLink = createMemo3(() => {
  ...
  createEffect2(() => onCleanup2(context.registerTriggerId(local.id)));
  return <ButtonRoot
...
function MenuContentBase(props) {
  ...
  createEffect3(() => onCleanup3(context.registerContentId(local.id)));
  onCleanup3(() => context.setContentRef(void 0));
  ...
  return <Show2 when={context.contentPresent()}><Show2
...
function MenuContent(props) {
  ...
  const rootContext = useMenuRootContext();
  const context = useMenuContext();
  const [local, others] = splitProps6(props, ["ref"]);
  createPreventScroll({
    element: () => ref ?? null,
...
import { createGenerateId as createGenerateId2, mergeDefaultProps as mergeDefaultProps6 } from "@kobalte/utils";
import { createSignal as createSignal3, createUniqueId as createUniqueId3 } from "solid-js";
...
function MenuGroup(props) {
  ...
  const [labelId, setLabelId] = createSignal3();
  const context = {
...
function MenuGroupLabel(props) {
  ...
  createEffect4(() => onCleanup4(context.registerLabelId(local.id)));
  return <Polymorphic
    as="span"
    id={local.id}
    aria-hidden="true"
...
function MenuItemDescription(props) {
  ...
  const [local, others] = splitProps8(mergedProps, ["id"]);
  createEffect5(() => onCleanup5(context.registerDescription(local.id)));
  return <Polymorphic
    as="div"
    id={local.id}
    {...context.dataset()}
...
import { mergeDefaultProps as mergeDefaultProps10 } from "@kobalte/utils";
import { Show as Show3, splitProps as splitProps9 } from "solid-js";
function MenuItemIndicator(props) {
  ...
  const [local, others] = splitProps9(mergedProps, ["forceMount"]);
  return <Show3 when={local.forceMount || context.isChecked()}><Polymorphic
    as="div"
    {...context.dataset()}
    {...others}
  /></Show3>;
...
import { mergeDefaultProps as mergeDefaultProps11, mergeRefs as mergeRefs5 } from "@kobalte/utils";
import {
...
function MenuItemLabel(props) {
  ...
  const [local, others] = splitProps10(mergedProps, ["ref", "id"]);
  createEffect6(() => onCleanup6(context.registerLabel(local.id)));
  return <Polymorphic
    as="div"
...
function MenuPortal(props) {
  const context = useMenuContext();
  return <Show4 when={context.contentPresent()}><Portal {...props} /></Show4>;
}
...
function MenuRadioGroup(props) {
  ...
  const [local, others] = splitProps11(mergedProps, [
    "value",
    "defaultValue",
    "onChange",
    "disabled"
...
import { mergeDefaultProps as mergeDefaultProps13 } from "@kobalte/utils";
...
function MenuRadioItem(props) {
  ...
  const [local, others] = splitProps12(mergedProps, ["value", "onSelect"]);
  const onSelect = () => {
    local.onSelect?.();
...
import { createUniqueId as createUniqueId5, splitProps as splitProps13 } from "solid-js";
function MenuRoot(props) {
  const optionalMenubarContext = useOptionalMenubarContext();
  const defaultId = `menu-${createUniqueId5()}`;
  ...
  const [local, others] = splitProps13(mergedProps, [
    "id",
    "modal",
    "preventScroll",
    "forceMount",
...
function MenuSubTrigger(props) {
  ...
  const mergedProps = mergeDefaultProps15(
    {
      id: rootContext.generateId(`sub-trigger-${createUniqueId6()}`)
    },
    props
  );
